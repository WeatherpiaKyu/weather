...
export default function KyrgyzProject(props) {
  const { shadowMouse, setShadowMouse } = useContext(QueryContext);

  const projectDetail = [
    {
      name: "Kyrgyzstan Mitigation",
      path: findPathByName(menuItem, ["프로젝트", "Kyrgyzstan", "Mitigation"]),
    },
    {
      name: "Kyrgyzstan Adaptation",
      path: findPathByName(menuItem, ["프로젝트", "Kyrgyzstan", "Adaptation"]),
    },
  ];
